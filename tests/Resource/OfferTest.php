...
final class OfferTest extends TestCase
{
    ...
    {
        $this->client->enqueue(new FakeResponseWithBody(200, '%PDF-offer', [
            'Content-Type' => 'application/pdf',
        ]));

        $response = $this->offers->downloadDocument('of1');

        self::assertSame('application/pdf', $response->contentType);
        self::assertStringContainsString('%PDF-offer', $response->toString());

        $sent = $this->client->sentRequests[0];
        self::assertSame('GET', $sent->getMethod());
        self::assertSame('/openapi/offer/document/of1', $sent->getUri()->getPath());
    }
    ...
    public function testGetBuildsPath(): void
    {
        ...
        $sent = $this->client->sentRequests[0];
        self::assertSame('GET', $sent->getMethod());
        self::assertSame('/openapi/offer/of1', $sent->getUri()->getPath());
    }
    ...
    public function testListBuildsQuery(): void
    {
        ...
        $this->offers->list(limit: 25, offset: 0);

        $sent = $this->client->sentRequests[0];
        self::assertSame('GET', $sent->getMethod());
        self::assertSame('/openapi/offer', $sent->getUri()->getPath());
    }
}
